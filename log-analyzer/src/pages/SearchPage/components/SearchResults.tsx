...
export const SearchResults = memo<SearchResultsProps>(
  ({
    ...
    return (
      <div
        ref={parentRef}
        className="flex-1 overflow-auto bg-bg-main scrollbar-thin"
      >
        {/* 表头 */}
        <div className="apple-material sticky top-0 z-10 grid grid-cols-[70px_160px_170px_1fr] border-b border-border-base px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.08em] text-text-muted">
            ...
            if (!log) {
              return (
                <div
                  ...
                  className="absolute left-0 top-0 grid w-full grid-cols-[70px_160px_170px_1fr] border-b border-border-subtle px-4 py-2"
                >
                  <div className="h-3 w-10 rounded bg-bg-elevated/60" />
                  <div className="h-3 w-32 rounded bg-bg-elevated/60" />
                  <div className="h-3 w-24 rounded bg-bg-elevated/60" />
                  <div className="h-3 w-3/4 rounded bg-bg-elevated/60" />
                </div>
              );
            }
            return (
          ...
          <div className="flex items-center justify-center h-full min-h-[200px]">
            {!isInitialized || workspaceLoading ? (
              ...
              <EmptyState
                ...
                description={t(
                  "search.empty_state.no_workspace_hint",
                  "请先创建或导入工作区以开始搜索日志"
                )}
              />
            ) : !query.trim() ? (
              <EmptyState
                ...
                description={t(
                  "search.empty_state.no_query_hint",
                  "在上方输入框中输入关键词进行搜索"
                )}
              />
            ) : (
              <EmptyState
                ...
                description={t(
                  "search.empty_state.no_results_hint",
                  "尝试调整搜索条件或关键词"
                )}
              />
            )}
