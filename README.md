...
- **高性能搜索体验**：Aho-Corasick 多模式匹配 + 磁盘直写分页，面向大结果集保持可交互
- **智能存储**：CAS（内容寻址存储）+ SQLite 元数据，自动去重，支撑嵌套压缩包与增量处理
- **全格式支持**：ZIP / TAR / GZ / RAR / 7Z，支持 7 层嵌套压缩包递归解析
- **完全离线**：所有数据本地处理，零网络传输，隐私安全有保障
- **可靠性保障**：Saga 事务模式、垃圾回收、缓存指标监控，确保数据完整性
...
| 交互式搜索主链路 | `search_logs` + `fetch_search_page`，磁盘直写分页避免一次性回传大结果集 |
...
- **SQLite 元数据**：存储文件、归档与工作区元信息；FTS5 / Tantivy 基础设施保留给索引相关链路
...
Linux 额外需要 GTK3/GTK4 开发库，详见 [Tauri 2 前置依赖](https://v2.tauri.app/start/prerequisites/)。
...
│           ├── search_engine/       # 搜索基础设施（Tantivy 索引、磁盘结果分页等）
...
| MetadataStore | `storage/metadata_store.rs` | SQLite 元数据管理 |
...
搜索：用户查询 → `search_logs` → `MetadataStore::get_all_files()` → CAS 读取内容 → `QueryExecutor` 匹配 → 磁盘分页返回
      ↓
...
| libloading | 0.8 | 预留给后续插件化实验的动态库加载依赖 |
| notify | 6.1 | 文件系统监听 |
