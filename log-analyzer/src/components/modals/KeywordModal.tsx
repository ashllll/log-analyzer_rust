...
import { Button, DialogSurface, Input } from "../ui";
...
const KeywordModal: React.FC<KeywordModalProps> = ({
  ...
  const handlePatternChange = useCallback(
    (id: string, field: "regex" | "comment", value: string) => {
      setPatterns((prev) => {
  ...
  const handleSave = useCallback(() => {
    ...
    const validPatterns = itemsToPatterns(
      patterns.filter((p) => p.regex.trim() !== "")
    );
    onSave({
  ...
  }, [name, color, patterns, initialData, onSave, validateForm]);

  return (
    <DialogSurface
      open={isOpen}
      onClose={onClose}
      ariaLabelledBy="keyword-dialog-title"
    ...
    >
      ...
      <div className="px-6 py-4 border-b border-border-base flex justify-between items-center bg-bg-sidebar">
        ...
        >
          ...
          {initialData
            ? t("keywords.modal.title_edit")
            : t("keywords.modal.title_new")}
        </h2>
        <Button
          variant="icon"
          icon={X}
          data-dialog-close
          aria-label={t("keywords.modal.cancel")}
        />
      </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div className="flex gap-2 h-9 items-center">
              {(["blue", "green", "orange", "red", "purple"] as ColorKey[]).map(
                (c) => (
                  <button
                    key={c}
                    type="button"
                    onClick={() => setColor(c)}
                    className={cn(
                      "h-6 w-6 cursor-pointer rounded-full border-2 transition-[border-color,opacity,transform] duration-150",
                      COLOR_STYLES[c].dot,
                      color === c
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            ...
            <Button
              ...
              onClick={() => {
                setPatterns((prev) => [
                  ...prev,
                  { id: generatePatternId(), regex: "", comment: "" },
                ]);
          ...
          {errors.patterns &&
            ...
            errors.patterns.length > 0 && (
              <div className="mb-2 p-2 bg-status-error/10 border border-status-error/50 rounded">
                {errors.patterns.map((error, index) => (
                  <div
                    key={index}
                    className="flex items-center gap-1 text-status-error text-xs"
                  >
                    <AlertCircle size={12} />
                    <span>{error}</span>
                  </div>
              ...
              <div key={p.id} className="flex gap-2 items-center group">
                <div className="flex-1">
                  <Input
                    value={p.regex}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      handlePatternChange(p.id, "regex", e.target.value)
                    }
                    onBlur={handlePatternBlur}
                    placeholder={t("keywords.modal.regex_placeholder")}
                    className={cn(
                      "font-mono text-xs",
                      errors.patterns?.[patterns.indexOf(p)] ||
