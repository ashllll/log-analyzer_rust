...
struct AppState {
    ...
    path_map: Arc<Mutex<HashMap<String, String>>>,  // 使用 Arc 优化内存
    file_metadata: Arc<Mutex<HashMap<String, FileMetadata>>>,  // 文件元数据映射
    workspace_indices: Mutex<HashMap<String, PathBuf>>,
    search_cache: Arc<Mutex<lru::LruCache<(String, String), Vec<LogEntry>>>>,  // 搜索缓存：(query, workspace_id) -> results
}
...
fn get_file_metadata(path: &Path) -> Result<FileMetadata, String> {
    ...
    let modified_time = modified.duration_since(SystemTime::UNIX_EPOCH)
...
fn save_index(
    ...
    eprintln!("[DEBUG] Index saved (compressed): {} ({} entries)", index_path.display(), path_map.len());
    Ok(index_path)
}

// 从磁盘加载索引（带解压，Windows 兼容，返回元数据）
fn load_index(index_path: &Path) -> Result<(HashMap<String, String>, HashMap<String, FileMetadata>), String> {
...
fn process_rar_archive(
    ...
    // 检查 unrar 命令是否可用
    let check = Command::new(unrar_cmd)
        .arg("-?")
        .output();
    ...
    eprintln!("[DEBUG] Extracting RAR: {} -> {}", path.display(), extract_path.display());
    ...
    eprintln!("[DEBUG] RAR extracted successfully: {}", extract_path.display());
    
    // 递归处理解压后的内容
    for entry in WalkDir::new(&extract_path).min_depth(1).into_iter().filter_map(|e| e.ok()) {
        let entry_path = entry.path();
        let relative = entry_path.strip_prefix(&extract_path)
...
fn process_path_recursive_with_metadata(
    ...
    if let Err(e) = process_path_recursive_inner_with_metadata(
        path, virtual_path, target_root, map, metadata_map, app, task_id
    ) {
        eprintln!("[WARNING] Failed to process {}: {}", path.display(), e);
        let _ = app.emit("task-update", TaskProgress {
            task_id: task_id.to_string(), 
...
fn process_path_recursive_inner(
    ...
    eprintln!("[DEBUG] regular file indexed: real_path={}, virtual_path={}", 
             path.display(), normalized_virtual);
    Ok(())
}
...
fn process_path_recursive_inner_with_metadata(
    ...
    if path.is_dir() {
        for entry in WalkDir::new(path).min_depth(1).max_depth(1).into_iter().filter_map(|e| e.ok()) {
            ...
            process_path_recursive_with_metadata(
                entry.path(), &new_virtual, target_root, map, metadata_map, app, task_id
            );
        }
        return Ok(());
    }
    ...
    if let Ok(metadata) = get_file_metadata(path) {
        ...
        eprintln!("[DEBUG] regular file indexed with metadata: real_path={}, virtual_path={}", 
                 path.display(), normalized_virtual);
    } else {
        eprintln!("[DEBUG] regular file indexed (no metadata): real_path={}, virtual_path={}", 
                 path.display(), normalized_virtual);
    }
...
async fn import_folder(
    ...
    let canonical_path = canonicalize_path(source_path)
        .unwrap_or_else(|e| {
            eprintln!("[WARNING] Path canonicalization failed: {}, using original path", e);
    ...
    {
        let mut temp_guard = state.temp_dir.lock()
    ...
    std::thread::spawn(move || {
        ...
        let result = panic::catch_unwind(panic::AssertUnwindSafe(|| {
            ...
            if let Some(ref temp_dir) = *temp_guard {
                ...
                let source_path = Path::new(&path);
                let root_name = source_path.file_name()
                    .unwrap_or_default()
                    .to_string_lossy();
                ...
                // 保存索引到磁盘（包含元数据）
                match save_index(&app_handle, &workspace_id_clone, &map_guard, &metadata_guard) {
                    Ok(index_path) => {
...
async fn search_logs(
    ...
    // 尝试从缓存获取
    {
        let mut cache_guard = search_cache.lock()
            .expect("Failed to lock search_cache");
    ...
    std::thread::spawn(move || {
        ...
        let files: Vec<(String, String)> = {
            let guard = path_map.lock()
                .expect("Failed to lock path_map");
            guard.iter()
                .map(|(k, v)| (k.clone(), v.clone()))
                .collect()
        };
        
        eprintln!("[DEBUG] Searching {} files with query: {}", files.len(), query);

        // 并行搜索（使用 Rayon）
        let mut all_results: Vec<LogEntry> = files.par_iter()
            .enumerate()
...
async fn load_workspace(
    ...
    *metadata_guard = file_metadata;
    ...
    eprintln!("[DEBUG] Loaded {} files with {} metadata entries", map_guard.len(), metadata_guard.len());
...
pub fn run() {
    ...
    eprintln!("[INFO] Rayon thread pool initialized with {} threads", num_cpus);
