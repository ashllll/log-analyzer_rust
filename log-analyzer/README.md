...
- 🗄️ **SQLite元数据**: 使用SQLite管理文件元数据，支撑CAS工作区与归档关系
...
- **系统依赖**: 根据您的操作系统安装 [Tauri 2 前置依赖](https://v2.tauri.app/start/prerequisites/)
...
| 功能 | 描述 |
|------|------|
| 📦 **多格式压缩包** | 支持`.zip`、`.tar`、`.tar.gz`、`.tgz`、`.gz`、`.rar`(无 sidecar 依赖) |
...
- 📊 **SQLite 元数据 + CAS** - 支撑导入、工作区和归档元信息管理
...
- ✅ **高效查询体验**: `search_logs` + 磁盘直写分页，适合大结果集逐页读取
- ✅ **嵌套支持**: 完美支持任意深度的嵌套压缩包
...
搜索流程:
用户查询 → `search_logs` → 获取工作区文件列表 → 从CAS读取内容 → 结果写入磁盘分页缓存 → 前端按需读取
```
...
[仓库](https://github.com/ashllll/log-analyzer_rust) · [问题反馈](https://github.com/ashllll/log-analyzer_rust/issues)
