...
pub fn parallel_extract_gz_files(
    ...
    let pool = rayon::ThreadPoolBuilder::new()
        ...
        .build()
        .map_err(|e| AppError::archive_error(format!("Failed to create thread pool: {}", e), None))
        .ok();
    ...
    let process_file = |file: &std::path::PathBuf| -> Result<ExtractionSummary> {
        ...
        let rt = tokio::runtime::Handle::try_current().map_err(|e| {
            AppError::archive_error(format!("No tokio runtime: {}", e), Some(file.clone()))
        })?;
    ...
    match pool {
        Some(pool) => pool.install(|| files.par_iter().map(process_file).collect()),
    ...
    stream::iter(files.into_iter().enumerate())
        .map(|(idx, file)| {
            ...
            async move {
                ...
                .await
                .map_err(|e| {
                    AppError::archive_error(format!("Task join error: {}", e), Some(file))
                })?
            }
        })
