...
const FileFilterSettings: React.FC<FileFilterSettingsProps> = ({
  ...
  useEffect(() => {
    ...
    const loadConfig = async () => {
      try {
        ...
        if (!controller.signal.aborted) {
          setError(
            t("file_filter.load_error", { error: getFullErrorMessage(err) })
          );
        }
      } finally {
        if (!controller.signal.aborted) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setError(
        t("file_filter.save_error", { error: getFullErrorMessage(err) })
      );
    } finally {
  ...
  return (
    <DialogSurface
      open={isOpen}
      onClose={onClose}
      ariaLabelledBy="file-filter-dialog-title"
    ...
    >
      ...
      <div className="p-6 overflow-y-auto flex-1 space-y-6">
        {error && (
          <div className="bg-status-error/10 border border-status-error/30 text-status-error px-4 py-2 rounded text-sm">
            {error}
          </div>
        )}
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="space-y-1">
              <p className="font-semibold">
                {t("file_filter.three_layer_strategy")}
              </p>
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  setConfig({
                    ...config,
                    binary_detection_enabled: e.target.checked,
                  })
                }
                className="sr-only peer"
              />
              <div className="w-11 h-6 bg-bg-subtle peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-primary rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-bg-card after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-bg-card after:border-border-base after:border after:rounded-full after:h-5 after:w-5 after:transition-transform after:duration-150 peer-checked:bg-primary"></div>
            </label>
            ...
            <div className="flex gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={() =>
                    setConfig({ ...config, mode: FilterMode.Whitelist })
                  }
                  className="w-4 h-4 text-primary bg-bg-main border-border-base focus:ring-primary"
                />
                <span className="text-sm text-text-main">
                  {t("file_filter.whitelist_mode")}
                </span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={() =>
                    setConfig({ ...config, mode: FilterMode.Blacklist })
                  }
                  className="w-4 h-4 text-primary bg-bg-main border-border-base focus:ring-primary"
                />
                <span className="text-sm text-text-main">
                  {t("file_filter.blacklist_mode")}
                </span>
              </label>
            </div>
            <p className="text-xs text-text-dim">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                variant="icon"
                icon={Plus}
                aria-label={t("file_filter.add_pattern")}
              />
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                variant="icon"
                icon={Plus}
                aria-label={t("file_filter.add_extension")}
              />
            </div>
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {config.allowed_extensions.map((ext) => (
                  <div
                    ...
                    className="flex items-center gap-1 px-2 py-1 bg-bg-sidebar border border-border-base rounded text-xs text-text-main"
                  >
                    <code className="text-status-success">.{ext}</code>
                    <button
                      onClick={() => removeExtension(ext)}
                      className="ml-1 text-text-dim hover:text-log-error transition-colors"
                      aria-label={t("file_filter.remove_extension", {
                        extension: ext,
                      })}
                    >
                      <Trash2 className="w-3 h-3" />
