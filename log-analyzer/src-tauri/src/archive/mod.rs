...
impl ArchiveManager {
     ...
     */
    pub fn new() -> Self {
        let mut handlers: Vec<Box<dyn ArchiveHandler>> = Vec::new();
        
        // 按优先级添加处理器
        handlers.push(Box::new(TarHandler));  // 先检查TAR（包括tar.gz等）
        handlers.push(Box::new(GzHandler));   // 再检查纯GZ
        handlers.push(Box::new(ZipHandler));  // 然后ZIP
        handlers.push(Box::new(RarHandler));  // 最后RAR
    ...
    pub async fn extract_archive(
        ...
        let handler = self.find_handler(source)
            .ok_or_else(|| crate::error::AppError::archive_error(
