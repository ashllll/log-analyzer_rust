...
**根本原因分析**:
虽然之前修复了 `insert_file()` 函数使用 `INSERT OR IGNORE + SELECT`，但测试仍然失败，说明：
1. 可能有其他插入路径没有使用这个逻辑
2. 或者测试代码本身有问题
3. 或者 UNIQUE 约束没有正确设置

**修复优先级**: 🔴 **P0 - 阻断发布**
**影响范围**: 所有导入功能、CAS 存储
**修复方案**: 需要全面检查所有 CAS 插入路径，确保 UNIQUE 约束生效

---

### 2. ❌ EventManager 内存泄漏 - **React StrictMode 不兼容**

**位置**: `log-analyzer/src/components/EventManager.tsx`

**问题描述**:
- 测试显示：5 次挂载周期产生了 15 次 mockListen 调用
- 期望：每周期 1 次订阅 + 1 次取消 = 5 次
- 实际：15 次订阅，说明没有正确清理

**失败的测试**:
```typescript
// EventManager.test.tsx: Memory Leak Prevention
expect(mockListen).toHaveBeenCalledTimes(5);  // Expected
Received number of calls: 15              // Actual - Memory leak!
```

**根本原因**:
React StrictMode 会双重挂载组件以检测副作用问题。如果 cleanup 不正确，会导致：
- 订阅叠加
- 事件监听器累积
- 内存泄漏

**修复优先级**: 🔴 **P0 - 生产环境内存泄漏**
**影响范围**: 所有使用 EventManager 的组件
**修复方案**:
1. 检查 EventManager 的 useEffect cleanup 逻辑
2. 确保使用 `useRef` 防止 StrictMode 重复初始化
3. 验证所有监听器都被正确取消
