...
基于 Rust + Flutter / Tauri + React 构建的现代化日志分析平台
...
Log Analyzer 是一款面向开发者和运维人员的**专业级桌面日志分析工具**，采用 Rust + Flutter / Tauri + React 现代技术栈打造，提供极致的性能体验和可靠的数据处理能力。
...
| **稳定** | 熔断自愈 | Circuit Breaker + 锁中毒自动恢复 |
| **安全** | 深度防御 | 路径验证 + 插件白名单 + ABI 验证 |
| **测试** | 80%+ 覆盖率 | 单元测试 + 集成测试 + 属性测试 (Proptest) |
...
| npm | 10.0+ | 包管理器 |
| Rust | 1.78+ | 系统编程语言 (MSRV) |
| Cargo | 随Rust安装 | Rust包管理器 |
| Flutter | 3.27+ / Dart 3.6+ | 跨平台UI框架（可选） |

**系统依赖**:
- **Linux**: GTK3/GTK4 开发库, libwebkit2gtk-4.0-dev
- **macOS**: Xcode Command Line Tools
- **Windows**: Microsoft C++ Build Tools (MSVC)

详细参考 [Tauri Prerequisites](https://tauri.app/v1/guides/getting-started/prerequisites)
...
# 3. 生成 FFI 桥接代码（配置见 frb_codegen.yaml）
...
│                      前端层 (Frontend)                       │
├───────────────────────────────┬─────────────────────────────┤
│      Flutter (移动端/桌面)      │    Tauri + React (桌面端)    │
│      (flutter_rust_bridge)    │    (tauri::command)         │
└───────────────────────────────┴─────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                      FFI / Command 层                        │
│         src/ffi/bridge.rs  |  src/application/commands/      │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                      应用服务层 (Application)                 │
│    src/application/ - 命令处理、工作流编排、服务协调          │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                      领域层 (Domain)                         │
│    src/domain/ - 核心业务逻辑：搜索、导出、日志分析            │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                      基础设施层 (Infrastructure)              │
│    src/infrastructure/ - 配置、持久化、外部服务集成            │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                      核心服务模块                            │
├─────────────┬──────────────┬──────────────┬─────────────────┤
│  src/archive/│src/search_engine/│ src/storage/ │ src/services/   │
│  归档处理    │ 搜索引擎      │ CAS存储      │ 业务服务        │
├─────────────┼──────────────┼──────────────┼─────────────────┤
│ src/task_manager/│ src/security/ │ src/monitoring/ │ src/state_sync/ │
│ 任务管理    │ 安全防护      │ 监控指标     │ 状态同步        │
└─────────────┴──────────────┴──────────────┴─────────────────┘
```
...
| 模块 | 功能 | 技术要点 |
|------|------|----------|
| `application/commands/search.rs` | 搜索命令 | 核心搜索逻辑，支持多模式匹配 |
| `application/commands/async_search.rs` | 异步搜索 | 可取消的异步搜索，流式结果 |
| `application/commands/http_api.rs` | HTTP API | axum 服务器，供 Flutter 调用 |
| `ffi/bridge.rs` | FFI 桥接 | flutter_rust_bridge 2.x 实现 |
| `search_engine/mod.rs` | 搜索引擎管理 | Tantivy引擎，<200ms响应 |
| `search_engine/concurrent_search.rs` | 并发搜索 | 流式处理，内存O(max_concurrent) |
| `search_engine/aho_corasick_searcher.rs` | Aho-Corasick匹配 | 多模式匹配 O(n+m) |
| `archive/mod.rs` | 归档处理 | 7层递归解压，流式处理 |
| `archive/fault_tolerance/` | 熔断自愈 | Circuit Breaker + 锁中毒恢复 |
| `storage/cas.rs` | 内容寻址存储 | SHA-256哈希，原子操作 |
| `storage/metadata_store.rs` | 元数据存储 | SQLite + FTS5 全文索引 |
| `services/file_change_detector.rs` | 变更检测 | SHA-256 哈希比较 |
| `security/path_validator.rs` | 路径验证 | 递归扫描防路径遍历 |
| `security/import_security.rs` | 导入安全 | 路径验证，炸弹检测 |
| `concurrency_safety/circuit_breaker.rs` | 熔断器 | Panic捕获与隔离 |
| `concurrency_safety/poison_recovery.rs` | 锁中毒恢复 | 自动恢复Poisoned Lock |
| `task_manager/` | 任务管理 | Actor模型异步任务 |
...
make build-ffi          # 构建FFI库 (cdylib)
make build-standalone   # 构建独立程序 (Tauri)

# 测试
make test               # 运行所有测试
cargo test --all-features              # 完整测试
cargo test --lib --all-features        # 单元测试
cargo test --test '*' --all-features   # 集成测试
cargo test -- --nocapture              # 显示输出

# 代码质量
make fmt                # 格式化代码
make fmt-check          # 检查格式
make lint               # 运行 Clippy (严格模式)
make lint-fix           # 自动修复警告
make check              # cargo check

# 依赖管理
make audit              # cargo-deny 审计
make outdated           # 检查过期依赖
make duplicates         # 检查重复依赖
make unused             # 查找未使用依赖
...
- ✅ `make fmt-check` / `cargo fmt --check` - 代码格式
- ✅ `make lint` / `cargo clippy -- -D warnings` - 静态分析
- ✅ `cargo test --all-features` - 单元/集成测试
- ✅ `npm run lint` - 前端检查
...
test = []              # 测试专用特性
ffi = []               # Flutter FFI支持
standalone = [         # Tauri独立应用
    "dep:tauri",
    "dep:tauri-build",
    ...
]
rar = ["dep:unrar"]    # RAR格式支持
telemetry = [          # 可观测性
    "dep:opentelemetry",
    ...
]
...
**常用 feature 组合**:
- `cargo build` - 默认构建 (FFI)
- `cargo build --features standalone` - Tauri桌面应用
- `cargo test --all-features` - 启用所有特性测试
...
**命名约定**:
- **模块/函数/变量**: `snake_case`
- **类型/Trait/枚举**: `CamelCase` (PascalCase)
...
// 3. 内部模块
crate::models::{FileMetadata, SearchQuery};
...
**错误处理**:
- **禁止使用** `unwrap()` / `expect()` 在生产代码
- 使用 `?` 传播错误
- 统一错误类型 `AppError` (位于 `error.rs`)
...
**绝对铁律**: Rust字段名 = Dart字段名 = JSON字段名

```rust
// Rust - snake_case
#[derive(Serialize)]
pub struct TaskInfo {
    pub task_id: String,        // 不是 taskId
