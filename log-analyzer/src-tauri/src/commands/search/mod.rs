...
use crate::commands::search::events::{
    emit_search_error, emit_search_id_event, SearchCompleteEvent, SearchProgressEvent,
    SearchResultBatchEvent, SearchSummaryEvent,
};
use crate::commands::search::filters::{
    CompiledSearchFilters, ParsedLineMetadata, SearchLineCandidate, SearchSegmentSummary,
};
...
pub(crate) fn remove_cancellation_token(
    tokens: &Arc<Mutex<HashMap<String, tokio_util::sync::CancellationToken>>>,
    search_id: &str,
) {
...
> {
    let workspace_dir = {
        if let Some(dir) = workspace_dirs.lock().get(workspace_id).cloned() {
            dir
        } else {
            resolve_workspace_dir(app_handle, workspace_id).map_err(|e| {
                ...
                CommandError::new("NOT_FOUND", e)
                    .with_help("Workspace may have been deleted. Try refreshing")
            })?
        }
    };
    ...
    let files = metadata_store
        ...
        .await
        .map_err(|e| {
            error!(%workspace_id, %e, "Failed to get pruned files");
            CommandError::new("DATABASE_ERROR", "Failed to access workspace files")
                .with_help("The database may be corrupted. Try reimporting")
        })?;
    if !disk_result_store.has_session(search_id) {
        if let Err(e) = disk_result_store.create_session(search_id) {
            ...
            return Err(
                CommandError::new("IO_ERROR", format!("Failed to create session: {e}"))
                    .with_help("Check app data dir is writable"),
            );
        }
    }
    Ok((files, cas))
...
async fn run_search_with_timeout(req: SearchExecutionRequest) -> Result<String, CommandError> {
    ...
    let cts = Arc::clone(&cancellation_tokens);
    let ds = Arc::clone(&disk_result_store);
    let timed_out = Arc::new(AtomicBool::new(false));
    let tfb = Arc::clone(&timed_out);
    ...
    match tokio::time::timeout(std::time::Duration::from_secs(search_timeout_secs), handle).await {
        Ok(Ok(())) => Ok(search_id),
        Ok(Err(e)) => {
            error!(%e, %search_id, "Search panicked");
            {
                cts.lock().remove(&search_id);
            }
            ds.remove_session(&search_id);
            Err(
                CommandError::new("INTERNAL_ERROR", format!("Search panicked: {e}"))
                    .with_help("Try simplifying your query"),
            )
        }
        Err(_) => {
            warn!(%search_id, "Search timed out after {}s", search_timeout_secs);
            ...
            ds.remove_session(&search_id);
            emit_search_id_event(&app_h, "search-timeout", &search_id);
            Err(CommandError::new(
                "TIMEOUT_ERROR",
                format!("Search timed out after {}s", search_timeout_secs),
            )
            .with_help("Use more specific terms"))
        }
    }
}
...
pub async fn search_logs_impl(
    ...
    let sm = Arc::clone(&state.search_metrics);
    ...
    let (raw, sq) = resolve_search_query(
        &query,
        structuredQuery,
        rc.case_sensitive,
        "search_logs_query",
    ...
    {
        let mut t = cts.lock();
        if let Some(old) = t.get(&sid) {
            old.cancel();
        }
        t.insert(sid.clone(), token.clone());
    }
    {
    ...
    let tantivy_prefetch = if let Some(ref mgr) = sem {
        ...
        match mgr
            .search_multi_keyword(&raw, require_all, Some(mr), None, None)
            .await
        {
            Ok(r) => Some(r.entries),
            Err(_) => None,
    ...
    tokio::spawn(async move {
        let r = run_search_with_timeout(SearchExecutionRequest {
            app_handle: ah.clone(),
            search_id: bid.clone(),
            files_for_search: files,
            cas,
            structured_query: sq,
            compiled_filters: cf,
            max_results: mr,
            regex_cache_size: rc.regex_cache_size.max(1),
            raw_terms: raw,
            search_metrics: sm,
            disk_result_store: ds,
            cancellation_token: token,
            cancellation_tokens: cts,
            search_timeout_secs: rc.timeout_seconds,
            search_thread_pool: tp,
            search_engine_manager: sem,
            tantivy_prefetch,
        })
        .await;
        if let Err(e) = r {
            if e.code != "TIMEOUT_ERROR" {
                emit_search_error(&ah, &bid, e.message);
            }
        }
    });
    Ok(sid)
...
fn execute_file_search(
    ...
    raw_terms: Vec<String>,
    search_metrics: Arc<Mutex<SearchMetrics>>,
    disk_store: Arc<la_search::DiskResultStore>,
    cancellation_token: tokio_util::sync::CancellationToken,
    cancellation_token_map: Arc<Mutex<HashMap<String, tokio_util::sync::CancellationToken>>>,
    search_thread_pool: Arc<rayon::ThreadPool>,
    timed_out: Arc<AtomicBool>,
    _search_engine_manager: Option<Arc<la_search::SearchEngineManager>>,
    tantivy_prefetch: Option<Vec<LogEntry>>,
) {
    ...
    let plan = match builder.build(&structured_query) {
        Ok(p) => p,
        Err(e) => {
            emit_search_error(&app_handle, &search_id, format!("Query error: {e}"));
            ...
            match disk_store.append_entries(&search_id, batch) {
                Ok(_) => {
                    if !to.load(Ordering::SeqCst) {
                        ...
                        let _ = app_handle.emit(
                            "search-result-batch",
                            SearchResultBatchEvent {
                                search_id: search_id.clone(),
                                entries: batch.clone(),
                                offset: count - batch.len(),
                                is_final: false,
                            },
                        );
                    }
                ...
                Err(_e) => {
                    if to.load(Ordering::SeqCst) || cancellation_token.is_cancelled() {
                        batch.clear();
                        return true;
                    }
                }
    ...
    if let Some(prefetch) = tantivy_prefetch {
        for entry in prefetch {
            ...
            if let Some(ref kw) = entry.matched_keywords {
                for k in kw {
                    *keyword_counts.entry(k.clone()).or_insert(0) += 1;
                }
            }
            let mut e = entry;
            e.id = results_count;
            batch_results.push(e);
            results_count += 1;
            if batch_results.len() >= batch_size && !flush(&mut batch_results, results_count) {
                break;
            }
        }
    ...
    'outer: for file_batch in files_for_search.chunks(10) {
        ...
        for file_results in batch {
            for mut entry in file_results {
                if results_count >= max_results {
                    let _ = flush(&mut batch_results, results_count);
                    was_truncated = true;
                    break 'outer;
                }
        ...
        if !flush(&mut batch_results, results_count) {
            break;
        }
    }
    if !cancellation_token.is_cancelled() {
        ...
        if !timed_out.load(Ordering::SeqCst) {
            let ks: Vec<la_core::models::search_statistics::KeywordStatistics> = raw_terms
                .iter()
                .map(|t| {
                    la_core::models::search_statistics::KeywordStatistics::new(
                        t.clone(),
                        keyword_counts.get(t).copied().unwrap_or(0),
                        results_count,
                    )
                })
                .collect();
            let _ = app_handle.emit(
                ...
                SearchSummaryEvent {
                    search_id: search_id.clone(),
                    summary: SearchResultSummary::new(results_count, ks, dur, was_truncated),
                },
            );
            let _ = app_handle.emit(
                "search-complete",
...
fn build_log_entry(
    ...
    let keywords = match_details.as_ref().map(|d| {
        d.iter()
            .map(|m| m.term_value.clone())
            .collect::<HashSet<_>>()
            .into_iter()
            .collect::<Vec<_>>()
    });
    LogEntry {
        ...
        file: vpath.into(),
        real_path: rpath.into(),
        line: line_number,
        content: line.into_owned().into(),
        tags: vec![],
        match_details,
        matched_keywords: keywords.filter(|k| !k.is_empty()),
    }
}

fn search_lines_direct<'a, I: IntoIterator<Item = (usize, Cow<'a, str>)>>(
    lines: I,
    vpath: &str,
    rpath: &str,
    ...
    let mut r = Vec::new();
    for (i, line) in lines {
        if let Some(dt) = builder.match_with_details(plan, line.as_ref()) {
            let m = ParsedLineMetadata::parse(line.as_ref(), false);
            r.push(build_log_entry(
                global_offset + i,
                i + 1,
                vpath,
                rpath,
...
fn flush_search_segment(
    ...
    if filters.segment_may_match(summary) {
        for c in segment.drain(..) {
            if !filters.matches_parsed_line_metadata(&c.metadata) {
                continue;
            } else if let Some(dt) = builder.match_with_details(plan, c.line.as_ref()) {
                results.push(build_log_entry(
                    global_offset + c.index,
                    c.index + 1,
                    vpath,
                    rpath,
                    c.line,
...
fn search_lines_with_segment_pruning<'a, I: IntoIterator<Item = (usize, Cow<'a, str>)>>(
    lines: I,
    vpath: &str,
    rpath: &str,
    builder: &QueryPlanBuilder,
    ...
    let mut r = Vec::new();
    let mut seg = Vec::with_capacity(SEARCH_SEGMENT_LINE_COUNT);
    let mut sum = SearchSegmentSummary::default();
    let nd = filters.has_time_filter();
    for (i, line) in lines {
        let m = ParsedLineMetadata::parse(line.as_ref(), nd);
        sum.record(&m);
        seg.push(SearchLineCandidate {
            index: i,
            line,
            metadata: m,
        });
        if seg.len() >= SEARCH_SEGMENT_LINE_COUNT {
...
fn search_lines_with_details<'a, I: IntoIterator<Item = (usize, Cow<'a, str>)>>(
    lines: I,
    vpath: &str,
    rpath: &str,
    builder: &QueryPlanBuilder,
    ...
    if filters.needs_segment_pruning() {
        ...
        search_lines_direct(lines, vpath, rpath, builder, plan, global_offset)
    }
}
...
fn search_single_file_with_details(
    file_identifier: &str,
    virtual_path: &str,
    cas_opt: Option<&la_storage::ContentAddressableStorage>,
    builder: &QueryPlanBuilder,
    plan: &ExecutionPlan,
    filters: &CompiledSearchFilters,
    global_offset: usize,
    ...
    if let Some(hash) = file_identifier.strip_prefix("cas://") {
        ...
        let cas = match cas_opt {
            Some(c) => c,
            None => {
                error!(%hash, %virtual_path, "CAS not provided");
                return results;
            }
        };
        if !cas.exists(hash) {
            warn!(%hash, %virtual_path, "Hash not in CAS");
            return results;
        }
        let content = match cas.read_content_sync(hash) {
            Ok(b) => b,
            Err(e) => {
                warn!(%hash, %virtual_path, %e, "Failed to read CAS");
                return vec![LogEntry {
                    id: global_offset,
                    timestamp: "0".into(),
                    level: "ERROR".into(),
                    ...
                    line: 0,
                    content: format!("[Search system: cannot read file - {e}]").into(),
                    tags: vec![],
                    match_details: None,
        ...
        results = search_lines_with_details(
            text.lines().enumerate().map(|(i, l)| (i, Cow::Borrowed(l))),
            virtual_path,
            file_identifier,
            builder,
            plan,
        ...
        use std::path::Path;
        if !filters.matches_file(virtual_path, Some(file_identifier)) {
            return results;
        }
        ...
        if let Ok(file) = File::open(file_identifier) {
            ...
            results = search_lines_with_details(
                reader.lines().enumerate().filter_map(|(i, lr)| match lr {
                    Ok(l) => Some((i, Cow::Owned(l))),
                    Err(e) => {
                        warn!(file = %file_identifier, line = i + 1, %e, "Failed to read line");
                        None
                    }
                }),
                virtual_path,
