...
mod tests {
    ...
    fn make_executor(
        content: &str,
        matches_per_line: usize,
    ) -> (SearchExecutor, Arc<CapturingResults>, Arc<CapturingEvents>) {
    ...
    fn empty_file_set_returns_zero_results() {
        let (executor, results, events) = make_executor("line1\nline2\n", 1);
    ...
    #[tokio::test]
    async fn single_file_below_batch_size_flushes_results() {
        let (executor, results, _events) = make_executor("line1\nline2\nline3\n", 1);
    ...
    // ========================================================================
    // Test 3: Batch flush boundary — content with more than BATCH_SIZE lines
    // ========================================================================
    ...
    async fn batch_flush_fires_at_boundary() {
        ...
        // Progress events are emitted via tokio::spawn — yield to let them fire
        for _ in 0..5 {
    ...
    // ========================================================================
    // Test 4: Truncation — max_results smaller than total matches
    // ========================================================================

    #[tokio::test]
    async fn truncation_at_max_results() {
        let (executor, results, _events) = make_executor("a\nb\nc\nd\ne\n", 1);
        let files = make_test_files();
    ...
    fn cancellation_stops_search_early() {
        ...
        // Should get 0 or very few results since cancelled before processing
        assert_eq!(outcome.total_count, 0);
        assert!(!outcome.was_truncated);
