...
pub struct PagedSearchResult {
    /// 当前页的数据
    pub results: Vec<super::log_entry::LogEntry>,
    /// 总结果数
    ...
    pub has_more: bool,
    /// 搜索摘要
    pub summary: super::search_statistics::SearchResultSummary,
    /// 搜索查询字符串（用于缓存匹配）
    pub query: String,
    /// 缓存的搜索ID
...
impl PagedSearchResult {
    /// 创建新的分页结果
    pub fn new(
        results: Vec<super::log_entry::LogEntry>,
        total_count: usize,
        page_index: i32,
        page_size: usize,
        summary: super::search_statistics::SearchResultSummary,
        query: String,
        search_id: String,
    ) -> Self {
    ...
    pub fn slice_results(
        entries: &[super::log_entry::LogEntry],
        page: i32,
        size: usize,
    ) -> Vec<super::log_entry::LogEntry> {
        if page < 0 || size == 0 {
