...
struct HandlerRegistry {
    handlers: Vec<Box<dyn crate::archive_handler::ArchiveHandler>>,
}
...
impl HandlerRegistry {
    ...
    fn register(&mut self, handler: Box<dyn crate::archive_handler::ArchiveHandler>) {
        self.handlers.push(handler);
    }

    /// Find a handler that can process the given file
    fn find_handler(&self, path: &Path) -> Option<&dyn crate::archive_handler::ArchiveHandler> {
        self.handlers
            .iter()
...
impl ExtractionEngine {
    ...
    fn create_handler_registry(&self) -> HandlerRegistry {
        ...
        registry.register(Box::new(crate::zip_handler::ZipHandler));
        registry.register(Box::new(crate::tar_handler::TarHandler));
        registry.register(Box::new(crate::gz_handler::GzHandler));
        registry.register(Box::new(crate::rar_handler::RarHandler));
        registry.register(Box::new(crate::sevenz_handler::SevenZHandler));
...
mod tests {
    ...
    fn test_extraction_policy_default() {
        ...
        assert_eq!(policy.max_file_size, 100 * 1024 * 1024); // 100MB
        assert_eq!(policy.max_total_size, 500 * 1024 * 1024); // 500MB
